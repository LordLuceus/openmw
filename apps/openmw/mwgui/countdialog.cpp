...
namespace MWGui
{
    ...
    void CountDialog::openCountDialog(const std::string& item, const std::string& message, const int maxCount)
    {
        ...
        // Select the whole pre-filled value so the first digit the user types
        // REPLACES it instead of appending. Without this the field opens as
        // (say) "20" with the cursor at the end, so typing "5" makes "205",
        // which NumericEditBox immediately clamps back to 20 -- making typing
        // appear to do nothing. With the text selected, "5" overwrites to "5".
        mItemEdit->setTextSelection(0, mItemEdit->getTextLength());
        ...
        // Up/Down then echo each new value via onEditValueChanged. Note we do
        // NOT need to suppress an echo here: setValue() above doesn't fire
        // eventValueChanged (only arrows/typing do), so the initial value is
        // announced once, by this say(). A suppress flag would instead leak to
        // the user's first Down press and silently swallow it.
    ...
    void CountDialog::onEditValueChanged(int value)
    {
        ...
        // or typing). This only fires for genuine user changes -- setValue() on
        // open does not raise eventValueChanged -- so no suppression is needed.
